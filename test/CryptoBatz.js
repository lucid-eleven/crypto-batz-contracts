...
describe("CryptoBatz contract", function () {
  ...
  var auctionStartPrice = ethers.utils.parseEther("1.666");
  var auctionBottomPrice = ethers.utils.parseEther("0.1");
  var auctionStepPrice = ethers.utils.parseEther("0.006525");
  var presaleSupplyLimit = 8000;
  ...
  let publicSaleStartTime = (+new Date("Thu Jan 20 2022 23:30:00 GMT+0000"))/1000;
  let publicSaleBottomTime = (+new Date("Fri Jan 21 2022 03:30:00 GMT+0000"))/1000;
  let publicSaleStepInterval = 60;
  ...
  describe("Deployment", function () {
    ...
    it("Should set the right sale config", async function () {
      ...
      expect(dutchAuctionConfig.stepInterval).to.equal(60);
  ...
  describe("Sale progression", function () {
    ...
    it("Should allow presale minting up to the presale supply limit", async function () {
      ...
      await expect(CryptoBatzContract.connect(addrs[1]).buyPresale(signature2, 1, 2000, {value: presaleMintPrice})).to.be.revertedWith('Not enough BATZ remaining');
      await expect(CryptoBatzContract.connect(addrs[2]).buyPresale(signature3, 1, 2000, {value: presaleMintPrice})).to.be.revertedWith('Not enough BATZ remaining');
    ...
    it("Should apply dutch auction price rules correctly", async function () {
      ...
      for (let i = 0; i < intervals; i++) {
        ...
        let gasEstimate = await CryptoBatzContract.connect(addr1).estimateGas.buyPublic(1, {value: currentPrice});
    ...
    it("Should allow public sale minting up to the max supply limit", async function () {
      ...
      await expect(CryptoBatzContract.connect(addr1).buyPublic(1, {value: auctionPrice})).to.be.revertedWith('Not enough BATZ remaining');
      await expect(CryptoBatzContract.connect(addr2).buyPublic(1, {value: auctionPrice})).to.be.revertedWith('Not enough BATZ remaining');
  ...
  describe("Withdrawing funds", function () {
    ...
    it("Should allow anyone to withdraw the entire balance in the contract into payee addresses", async function () {
      ...
      expect(await ethers.provider.getBalance("0xaDC6A7985036531c394B6dF054666C51dE29b9a9")).to.equal(expectedBalance.mul(50).div(100));
      expect(await ethers.provider.getBalance("0x76bf7b1e22C773754EBC608d92f71cc0B5D99d4B")).to.equal(expectedBalance.mul(45).div(100));
      expect(await ethers.provider.getBalance("0x8b8AbE9CEC13CF5050517816B2E95722E711818e")).to.equal(expectedBalance.mul(5).div(100));
    });
    ...
    it("Should allow direct transfers into the contract and withdrawal", async function () {
      ...
      let bal3 = await ethers.provider.getBalance("0x8b8AbE9CEC13CF5050517816B2E95722E711818e");
      ...
      expect(await ethers.provider.getBalance("0xaDC6A7985036531c394B6dF054666C51dE29b9a9")).to.equal(expectedBalance.mul(50).div(100).add(bal1));
      expect(await ethers.provider.getBalance("0x76bf7b1e22C773754EBC608d92f71cc0B5D99d4B")).to.equal(expectedBalance.mul(45).div(100).add(bal2));
      expect(await ethers.provider.getBalance("0x8b8AbE9CEC13CF5050517816B2E95722E711818e")).to.equal(expectedBalance.mul(5).div(100).add(bal3));
    });
